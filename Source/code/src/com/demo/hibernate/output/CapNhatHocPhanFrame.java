...
public class CapNhatHocPhanFrame extends JDialog implements ActionListener {
	...
	CapNhatHocPhanFrame(JFrame parent, HocPhan hp) {
		super(parent, true);
		_parent = parent;
		_hp = HocPhanDao.layThongTinHocPhan(hp.getMaHP());
		Container con = this.getContentPane();
		// Title
		JPanel titlePn = createTitle("         CHỈNH SỬA HỌC PHẦN         ");
		// Content
...
//		labelForSlang.setForeground(new Color(127, 127, 127, 50));
		maHP = new JTextField(_hp.getMaHP(), 20);
	...
	JPanel createMonHoc() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (list.get(i).getMaMH().equals(_hp.getMonHoc().getMaMH())) {
				pos = i;
			}
		}
	...
	JPanel createDotDKHP() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (list.get(i).getMaDot().equals(_hp.getDotDKHP().getMaDot())) {
				pos = i;
			}
		}
	...
	JPanel createSlot() {
		...
		labelForSlang.setFont(new Font("", Font.PLAIN, 18));
//		labelForSlang.setForeground(new Color(127, 127, 127, 50));
		slot = new JSpinner(new SpinnerNumberModel(_hp.getSlotToiDa(), 1, 200, 1));
...
//		labelForSlang.setPreferredSize(new Dimension(175, 18));
	...
	JPanel createGVLT() {
		...
		labelForSlang.setFont(new Font("", Font.PLAIN, 18));
//		labelForSlang.setForeground(new Color(127, 127, 127, 50));
		tenGVLT = new JTextField(_hp.getTenGVLT(), 20);
	...
	JPanel createPhong() {
		...
		labelForSlang.setFont(new Font("", Font.PLAIN, 18));
//		labelForSlang.setForeground(new Color(127, 127, 127, 50));
		tenPhong = new JTextField(_hp.getTenPhong(), 20);
	...
	JPanel createLop() {
		...
		labelForSlang.setFont(new Font("", Font.PLAIN, 18));
//		labelForSlang.setForeground(new Color(127, 127, 127, 50));
		lop = new JTextField(_hp.getMaLop(), 20);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == quayLaiBtn) {
			...
			HocPhan hp = new HocPhan(maHPS, tenGVLTS, thuS, _hp.getSlot(), caS, tenPhongS, lopS, slotS, mh, dot);
			if (HocPhanDao.updateHocPhan(hp)) {
				JOptionPane.showMessageDialog(this, "Cập nhật  học phần thành công");
			} else {
				JOptionPane.showMessageDialog(this, "Cập nhật  học phần không thành công", "Cập nhật  phần thất bại",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			this.dispose();
